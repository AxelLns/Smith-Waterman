...
public class GUI {
    ...
    private void initialize() {
        ...
        outputArea.setPreferredSize(new Dimension(470, 385)); // Set preferred size
        outputArea.setAlignmentX(Component.CENTER_ALIGNMENT);

        JScrollPane scrollPane = new JScrollPane(outputArea);
        scrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
        scrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        scrollPane.setPreferredSize(new Dimension(480, 395)); // Set preferred size
        ...
        outputArea.setEditable(false);
        outputArea.setFont(new Font("Monospaced", Font.PLAIN, 12));
        outputArea.setPreferredSize(new Dimension(470, 385));
        frame.getContentPane().add(outputArea);
        ...
        scrollPane.setPreferredSize(new Dimension(480, 395));  // Optional, set preferred size if needed
    ...
    private void runSmithWaterman() {
        ...
        if (seq1.equals("") || seq2.equals("") || match.equals("") || mismatch.equals("") || gap.equals("")) {
            ...
            int score = Algorithms.SmithWatermanAlgorithm(seq1, seq2, matrix, Integer.parseInt(match), Integer.parseInt(mismatch), Integer.parseInt(gap));
    ...
    /*private void runSmithWaterman() {
        ...
        Style style = doc.addStyle("Monospaced", null);
        ...
        try {
            ...
            for (int i = 0; i < matrix.length; i++) {
                for (int j = 0; j < matrix[i].length; j++) {
                    ...
                    Style cellStyle = doc.addStyle("Color_" + value, null);
                    StyleConstants.setForeground(cellStyle, (value == 0 ? black : colors[value % 6]));

                    // Use String.format to align numbers
                    String formattedValue = String.format("%2d", value);
                    doc.insertString(doc.getLength(), formattedValue + " ", doc.getStyle("Color_" + value));
                }
                doc.insertString(doc.getLength(), "\n", doc.getStyle("Monospaced"));
            }
