public class Algorithms {
    ...
    public static int SmithWatermanAlgorithm(String sequence1, String sequence2, int[][] matrix, int match,
        ...
        // Display the matrix
        System.out.print(Const.white + "| S |    | ");
        for (int i = 0; i < sequence2.length(); i++) {
            System.out.printf(Const.white + "%2c | ", sequence2.toCharArray()[i]);
        }
        ...
        for (int i = 0; i < sequence1.length() + 1; i++) {
            ...
            for (int j = 0; j < sequence2.length() + 1; j++) {
                System.out.printf((matrix[i][j] == 0 ? Const.white : Const.colors[matrix[i][j] % 6]) + "%2d | ",
                        matrix[i][j]);
            }
            System.out.print(Const.white + "\n");
        }
